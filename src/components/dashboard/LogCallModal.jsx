...
const LogCallModal = ({ open, onClose, subject }) => {
  if (!open) return null;

  return (
    <>
      <style>{`
        @keyframes logCallBackdropIn { from { opacity: 0; } to { opacity: 1; } }
        @keyframes logCallIn { from { opacity: 0; transform: translate(-50%, -48%) scale(0.97); } to { opacity: 1; transform: translate(-50%, -50%) scale(1); } }
      `}</style>

      <div onClick={onClose} style={{
        position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.35)",
        backdropFilter: "blur(3px)", WebkitBackdropFilter: "blur(3px)",
        animation: "logCallBackdropIn 0.25s ease-out both",
      }} />
      ...
      <div style={{
        ...
        animation: "logCallIn 0.3s cubic-bezier(0.16,1,0.3,1) both",
      }}>
        <div style={{ padding: "18px 24px 14px", borderBottom: "1px solid #F3F4F6", display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexShrink: 0 }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: "#111827" }}>Log Call</h2>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            ...
            <textarea rows={3} placeholder="What was the result of the call?" style={{ ...inputStyle, resize: "vertical" }} />
          </div>
          <div>
        ...
        <div style={{ padding: "16px 24px", borderTop: "1px solid #F3F4F6", display: "flex", justifyContent: "flex-end", gap: 10, flexShrink: 0, flexWrap: "wrap" }}>
          ...
          <button onClick={onClose} style={{ padding: "10px 18px", border: "1px solid #2563EB", borderRadius: 8, background: "#fff", fontSize: 13, fontWeight: 600, color: "#2563EB", cursor: "pointer", fontFamily: FONT }}>Save Call Log</button>
          <button onClick={onClose} style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 18px", border: "none", borderRadius: 8, background: "#2563EB", fontSize: 13, fontWeight: 600, color: "#fff", cursor: "pointer", fontFamily: FONT }}>
