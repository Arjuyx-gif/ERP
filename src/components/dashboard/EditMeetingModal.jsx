...
const EditMeetingModal = ({ open, onClose, meeting }) => {
  if (!open) return null;

  return (
    <>
      <style>{`
        @keyframes editMeetingBackdropIn { from { opacity: 0; } to { opacity: 1; } }
        @keyframes editMeetingIn { from { opacity: 0; transform: translate(-50%, -48%) scale(0.97); } to { opacity: 1; transform: translate(-50%, -50%) scale(1); } }
      `}</style>

      <div onClick={onClose} style={{
        position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.35)",
        backdropFilter: "blur(3px)", WebkitBackdropFilter: "blur(3px)",
        animation: "editMeetingBackdropIn 0.25s ease-out both",
      }} />
      ...
      <div style={{
        ...
        animation: "editMeetingIn 0.3s cubic-bezier(0.16,1,0.3,1) both",
      }}>
        {/* Header */}
        <div style={{ padding: "18px 24px 14px", borderBottom: "1px solid #F3F4F6", display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexShrink: 0 }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: "#111827" }}>Edit Meeting</h2>
        ...
        {/* Body */}
        <div className="thin-scrollbar" style={{ flex: 1, overflowY: "auto", padding: "20px 24px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
          <Field label="Organization"><input type="text" style={inputStyle} /></Field>
          <Field label="Address"><input type="text" style={inputStyle} /></Field>
          ...
          <Field label="Meeting Agenda"><textarea rows={3} style={{ ...inputStyle, resize: "vertical" }} /></Field>
          <Field label="Contact Person"><input type="text" style={inputStyle} /></Field>
          <Field label="Contact Number"><input type="text" style={inputStyle} /></Field>
          <Field label="Contact Mail ID"><input type="email" style={inputStyle} /></Field>
          <Field label="Remarks"><textarea rows={3} placeholder="Any additional notes..." style={{ ...inputStyle, resize: "vertical" }} /></Field>
        </div>
        ...
        <div style={{ padding: "16px 24px", borderTop: "1px solid #F3F4F6", display: "flex", justifyContent: "flex-end", gap: 10, flexShrink: 0 }}>
          ...
          <button onClick={onClose} style={{ padding: "10px 24px", border: "none", borderRadius: 8, background: "#2563EB", fontSize: 13, fontWeight: 600, color: "#fff", cursor: "pointer", fontFamily: FONT }}>Save Changes</button>
        </div>
      </div>
    </>
  );
