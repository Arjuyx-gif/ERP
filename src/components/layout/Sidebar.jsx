...
const Sidebar = () => {
  ...
  return (
    <div style={{
      width: 200, flexShrink: 0, background: "#2D3748",
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "8px 0" }}>
        {SIDEBAR_NAV.map(item => (
          <div key={item.label}>
            <div
              onClick={() => item.expandable && setExpanded(p => ({ ...p, [item.label]: !p[item.label] }))}
              style={{
                display: "flex", alignItems: "center", gap: 10, padding: "9px 16px",
                cursor: item.expandable ? "pointer" : "default",
                background: item.active && !item.expandable ? "#2979FF" : "transparent",
                borderRadius: item.active && !item.expandable ? 8 : 0,
                margin: "1px 8px",
              }}
            >
              <span style={{ display: "flex", alignItems: "center" }}>
                <DynamicIcon
                  name={item.iconName}
                  size={18}
                  color={item.active && !item.expandable ? "#fff" : "#A0AEC0"}
                />
              </span>
              <span style={{
                fontSize: 13, flex: 1, fontWeight: item.active ? 600 : 400,
                color: item.active && !item.expandable ? "#fff" : "#CBD5E0",
              }}>
                {item.label}
              </span>
              {item.expandable && (
              ...
              <div>
                {item.children.map(child => (
                  <div key={child.label} style={{
                    ...
                    borderRadius: child.active ? 8 : 0,
                    margin: child.active ? "1px 8px" : "0",
                    cursor: "pointer",
                  }}>
                    <span style={{
                      fontSize: 12,
                      color: child.active ? "#fff" : "#A0AEC0",
